...
fetch('Img/treelove.svg')
  ...
  .then(svgText => {
    ...
    const heartPaths = allPaths.filter(el => {
      ...
      return style.includes('#fc5858ff') || style.includes('#c14a1fff');
    });
...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Para la señorita mas hermosa:\n\nGracias por acompañarme y por hacer que mis días estén llenos de cariño.\n\nTe amo más de lo que puedo explicar con palabras.\n\n Feliz dia a la novia mas bella, hermosa, linda y unica <3. `;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con mucho amor y cariño, Yhntn";
...
function showCountdown() {
  ...
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2025-02-14T00:00:00');
  ...
  function update() {
    ...
    container.innerHTML =
      `Llevamos juntos mas de: <b>${days}</b> días<br>` +
      `Te quiero mucho M<3`;
